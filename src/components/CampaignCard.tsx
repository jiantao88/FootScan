...
export const CampaignCard: React.FC<CampaignCardProps> = ({ campaignAddress }) => {
    ...
    const { data: campaignName, isLoading: isLoadingName, error: nameError } = useReadContract({
        contract: contract,
        method: "function name() view returns (string)",
        params: []
    });

    const { data: campaignDescription, isLoading: isLoadingDescription, error: descError } = useReadContract({
        contract: contract,
        method: "function description() view returns (string)",
        params: []
    });

    const { data: goal, isLoading: isLoadingGoal, error: goalError } = useReadContract({
        contract: contract,
    ...
    const { data: deadline, isLoading: isLoadingDeadline, error: deadlineError } = useReadContract({
        contract: contract,
    ...
    // If any error occurs
    React.useEffect(() => {
        const errors = [nameError, descError, goalError, balanceError, ownerError, deadlineError].filter(Boolean);
        if (errors.length > 0) {
            console.error("Contract errors:", errors);
            setError("Failed to load campaign data");
        }
    }, [nameError, descError, goalError, balanceError, ownerError, deadlineError]);
